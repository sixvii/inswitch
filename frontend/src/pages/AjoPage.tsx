...
const AjoPage = () => {
  ...
  const buildOrderFromManualRank = (group: SavingsGroup, rankByUsername: Record<string, number>) => {
    ...
    const remaining = acceptedMembers.reduce<Record<string, number>>((acc, member) => {
      acc[member.ajoUsername] = Math.max(member.slots, 1);
      return acc;
    }, {});
    ...
    sortedUsernames.forEach((username) => {
      if ((remaining[username] || 0) <= 0) return;
      expandedOrder.push(username);
      remaining[username] -= 1;
    });

    while (expandedOrder.length < acceptedMembers.reduce((sum, member) => sum + Math.max(member.slots, 1), 0)) {
      let pushed = false;
      sortedUsernames.forEach((username) => {
        if ((remaining[username] || 0) <= 0) return;
        expandedOrder.push(username);
  ...
  if (view === 'details' && selectedGroup) {
    ...
    const selectedPeriodIndex = selectedDatePeriodKey
      ...
      : undefined;
    const scheduledRecipientUsername = selectedPeriodIndex !== undefined && activePayoutOrder.length > 0
      ? activePayoutOrder[selectedPeriodIndex % activePayoutOrder.length]
      : null;
    ...
    return (
      <div className="py-4 md:px-0 px-2 animate-fade-in">
            ...
            {(isCreator || currentMember?.accepted) && (
              <div className="space-y-4 mb-4">
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {isCreator && (
                    <div className="bg-card rounded-[10px] p-3 border border-border flex items-start justify-between gap-3 min-h-[110px]">
                      <div>
                ...
                {isCreator && (
                  <div className="bg-card rounded-[10px] p-4 border border-border">
        ...
        {isCreator && (
          <div className="bg-card rounded-[10px] p-4 mb-4 border border-border">
            <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
              <img src="/logo.svg" alt="logo" className="w-4 h-4 object-contain" />
        ...
        {isCreator && (
          <div className="mt-6 bg-card rounded-[10px] p-4 border border-border">
