...
import { fetchBackendRequests } from '@/lib/backendApi';
...
const RequestsPage = () => {
  ...
  const matchedRecipient = users.find((user) => user.accountNumber === requestedFromAccount.trim());
  const requestedFromName = matchedRecipient ? `${matchedRecipient.firstName} ${matchedRecipient.lastName}` : '';
